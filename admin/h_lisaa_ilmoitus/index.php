...
// Tarkistaa ettei osa kentistä lähde tyhjinä.
...
/* Hallinnoinnin sivuilla tämä ehkä voisi jäädä tyhjäksi?
Makuasia?
...
if (empty($jakaja)) {
    $tyhja_kentta_virhe = 'Tähdellä merkityt kentät tulee täyttää.';
    $jakaja_virhe = 'Täytä tämä kenttä.';
    $ilmoitus_lahtee = 0;
}
...
// Tarkistaa että osa kentistä on oikeassa muodossa.
...
if (!empty($nettisivu) && !preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i",$nettisivu)) { 
    ...
    /*  Päivä- ja kellonaikatarkistuksia.

    Lieköhän nämä tarkistukset ovat tarpeellisia hallinnoinin sivulla?
    Toistaiseksi eivät ole käytössä.

    $min_ilmoitus_poistuu = date('Y-m-d');
    $max_ilmoitus_poistuu = date('Y-m-d', strtotime('+1 year'));

if ($ilmoitusPoistuuAutomaattisestiPvm != '' 
    && $ilmoitusPoistuuAutomaattisesti != '00.00.0000'
    ...
    $ilmoitusPoistuuAutomaattisesti_virhe = 'Jos lisäät päivän, sen 
    tulee olla aikaisintaan tänään ja korkeintaan yksi vuosi tästä 
    päivästä lukien.';
    $ilmoitus_lahtee = 0;
}
