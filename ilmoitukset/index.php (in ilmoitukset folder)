...
<?php
  ...
  if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
      $osasto = $row['elokuvat'] . $row['improvisointi'] . $row['keskustelu'] . $row['kirjallisuus'] . 
                $row['kuvataide'] . $row['kasityot'] . $row['leikkiminen'] . $row['liikunta'] . 
                $row['luento'] . $row['musiikki'] . $row['nayttelyt'] . $row['oppiminen'] . 
                $row['pelaaminen'] . $row['politiikka'] . $row['runonlausunta'] . $row['sadunkerronta'] . 
                $row['tanssi'] . $row['tarinankerronta'] . $row['teatteri'] . $row['tyopaja'] . 
                $row['urheilu'] . $row['uskonto'] . $row['yhdessa_oleminen'] . $row['muut_tapahtumat'] . 
                $row['aikuiset'] . $row['lapset'] . $row['nuoret'] . $row['seniorit'] . $row['miehet'] . 
                $row['naiset'] . $row['elektroniikka'] . $row['huonekalut'] . $row['kodinkoneet'] . 
                $row['vaatteet'] . $row['muut_annetaan_tavaroita'] . $row['liikunta_2'] . $row['musiikki_2'] . 
                $row['nayttelyt_2'] . $row['paikat'] . $row['tilat'] . $row['urheilu_2'] . 
                $row['muut_annetaan_tekemista'] . $row['elektroniikka_2'] . $row['huonekalut_2'] . $row['kodinkoneet_2'] . 
                $row['vaatteet_2'] . $row['muut_pyydetaan_tavaroita'] . $row['liikunta_3'] . $row['musiikki_3'] . 
                $row['nayttelyt_3'] . $row['paikat_3'] . $row['tilat_3'] . $row['urheilu_3'] . 
                $row['muut_pyydetaan_tekemista']; 

      $escape_desc = htmlspecialchars($row["sisalto"], ENT_QUOTES, "UTF-8");
      $public_desc = html_entity_decode($escape_desc, ENT_QUOTES);  

      $strip_desc = strip_tags($row["sisalto"]);
      $meta_desc = htmlspecialchars($strip_desc, ENT_QUOTES, "UTF-8");
...
<head>
  ...
  <meta property="fb:app_id" content="your fb abb id" /> 
...
<body>
  ...
  <?php
    ...
    include "../includes/navigointi_palkki.php";
  ?>
      ...
      ?>
        ...
        <div class="vasen_puoli">      
          ...
          <img class="koko_ilmoitus_iso_kuva" src='<?php if ($row["source"] != "lomake" && $row["source"] != "h_lomake" &&  $row["kuva1"] != "oletuskuva_1234.jpg") {echo htmlspecialchars($row["kuva1"], ENT_QUOTES, "UTF-8"); } else {echo "https://jaetaan.net/kuvat/pienet_kuvat/" . htmlspecialchars($row["kuva1"], ENT_QUOTES, "UTF-8"); } ?>' width="100%" height="auto">
            ...
            <div class="trythis">
            <p class='koko_ilmoitus_teksti koko_ilmoitus_sisalto'><?php if ($row['source'] == 'linked_events') { 
              echo $public_desc; 
              } 
              else { 
                 echo htmlspecialchars($row["sisalto"], ENT_QUOTES, "UTF-8"); 
               } ?></p>
            </div>
            <br>
            ...
            <?php 
              ...
              if ($row['ylaosasto'] == 'Tapahtumat'
              && $row['jakaja'] != '') {
                echo '<br> <p class="koko_ilmoitus_teksti"><b>Järjestäjä: </b>' . htmlspecialchars($row['jakaja'], ENT_QUOTES, 'UTF-8') . '</p>'; 
              }   
...
' . $viesti;   
            ...
            <div id='viesti_ilmoittajalle_lomake_katoaa_II'>
              ...
              <h2 class='koko_ilmoitus_otsikko'>
                Lähetä viesti <?php if($row['jakaja'] != '') {echo 'jakajalle';} 
                                    if($row['jakaja'] == '') {echo 'ilmoittajalle';} ?>
              </h2>
              ...
              <form action='https://www.jaetaan.net/ilmoitukset/index.php?urlKokoIlmoitus=<?php echo $urlMuokkaaIV ?>#ankkuri' method='post' enctype='multipart/form-data'>
                ...
                <br>
                <input class="yhteydenottolomake" type="text" name="aihe" maxlength="100" value="<?php echo $aihe;?>">
  ...
  <body>
    ...
    <div class="keski_osa"> 
      ...
      <br><br> 
      <p class="info_teksti">Etsimääsi ilmoitusta ei valitettavasti löydy.</p>
      <br>

      <p class="info_teksti">Syynä tähän voi olla se, että ilmoitus on vanhentunut 
                             ja siksi poistettu Jaetaan.netistä.</p>
    
      <p class="info_teksti">Kannattaa myös tarkistaa, että ilmoituksen osoite on 
                             kirjoitettu oikein.</p>
      <br>
      <p class="info_teksti">Klikkaa yllä olevaa Etusivu-nappia päästäksesi etusivulle.</p>
      <br><br>
